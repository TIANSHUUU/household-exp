...
| 路径 | 是什么 | 建议 |
|---|---|---|
| `requirement.md` | 家庭账单页（`index.html`）最初的需求描述，含用户手写的一个月开支流水样例 | 提交进 `docs/`。是这个页面为什么长这样的唯一记录 |
| `shopping_list.md` | 购物清单页（`shopping.html`）最初的需求描述 | 同上 |
| `.claude/settings.json` | Claude Code 的项目级 Bash 权限白名单，没有任何密钥 | 提交。让两台电脑上的权限一致，少点授权弹窗 |
| `recipe-details/` | **食谱录入的源料目录**，见下 | 需要决定，见下 |
...
用户把配方和照片丢进这里，然后录进网页。**这里的照片是母版**——Storage 里那份是转过 sRGB 的衍生品（见第五节 HDR 问题）。母版只存在于本地，机器坏了就没了。

当前一个食谱约 732K。50 个食谱约 36MB——git 扛得住，但会一直涨。

两条路：

- **提交**：母版有备份，代价是仓库随食谱数量线性变大
- **gitignore**：仓库保持精简，代价是原图只在本地，需要用户自己做备份

没有明显更优解，取决于用户是否有别的照片备份方案。**在用户明确表态前不要擅自决定**——这两条路一旦选了再换都很麻烦（提交过的大文件即使删掉也留在 git 历史里）。
